...
library(gganimate)
p <- 
  ggplot(data = nyc_cvrisk_joined, aes(x = chd_value, y = data_value, frame = short_question_text, label = short_question_text)) +
  geom_point(data = nyc_cvrisk_joined, aes( 
                                           size = population_count, colour = short_question_text ),
             alpha = 0.5) + 
  
  labs(title = {nyc_cvrisk_joined$short_question_text},
