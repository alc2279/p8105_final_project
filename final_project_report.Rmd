...
animate(pp, fps = 1, height = 600, width = 1000, renderer = gifski_renderer())
...
#Additional Analysis
We used the `shinyApp` and `leaflet` package to perform visualization of geographic distribution of several risk factors and outcomes in New York City.

We also used `shinyApp` and `plotly` package to build a program to calculate individual risk of heart disease using the Framingham Risk Score
