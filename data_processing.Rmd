...
p <- 
  ...
  geom_point(data = nyc_cvrisk_joined, aes(x = chd_value, y = data_value, 
                                           size = population_count, colour = measure_id),
             alpha = 0.5) + 
  ...
  # gganimate parts
  transition_states(measure_id, transition_length = 1, state_length = 5000, wrap = TRUE) +
  enter_fade() +
  exit_fade()
...
p <- 
  ...
  # gganimate parts
  transition_states(measure_id, fps = 0.5) +
  enter_fade() +
  exit_fade()
