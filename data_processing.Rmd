...
```{r, eval = FALSE}
## Annie eda - this is all exploratory, for final clean dataset, look at next data chunk
nyc_cvrisk = 
  cvrisk %>% 
  filter(state_desc == "New York", city_name == "New York")
...
## I think I am feeling relatively comfortable with what's in the data; can start visualizing it
...
```{r}
## Annie EDA - cleaner version of data
...
nyc_cvrisk_2015 = 
  ...
  mutate(latitude = str_replace(latitude, "\\(", ""),
         longitude = str_replace(longitude, "\\)", "")) %>% 
  select(latitude, longitude, population_count, measure_id, data_value)
...
## use same tidiyng for full dataset (need to that same rules apply!)
cvrisk_2015 = 
  cvrisk %>% 
  filter(geographic_level == "Census Tract",
         !is.na(data_value),
